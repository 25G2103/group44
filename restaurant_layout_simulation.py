...
VISIBLE_DISTANCE = 9.0         # 入口からこの距離以内の卓は「見える席」
...
def create_layout_seats():
    ...
    def add(kind, x, y, w, h, capacity):
        nonlocal index
        cx, cy = x + w / 2, y + h / 2
        d = math.hypot(cx - ENTRANCE_POS[0], cy - ENTRANCE_POS[1])
        is_window = d <= VISIBLE_DISTANCE
    ...
    # --- 2人席：3ブロック、各ブロック仕切りで2列×2行 ---
    block_starts = [4.2, 7.6, 11.0]
    ...
    for i in range(n12):
        ...
        add("table4", x, row12_y, w12, row12_h, 4)
...
def run_simulation(n_steps=N_STEPS, spawn_interval=SPAWN_INTERVAL, seed=RANDOM_SEED,
    ...
    road_x_start = FLOOR_W + 6.0     # 通行人は右端から出現
...
def _draw_static_layout(ax, seats):
    ...
    # 見える範囲の目安円（入口中心）
    view_circle = mpatches.Circle(ENTRANCE_POS, VISIBLE_DISTANCE, fill=False,
                                   edgecolor="#5a9bd4", linewidth=1.2,
